...
class TrainVPGDiffusionAgent(TrainPPOAgent):
        ...
        while self.itr < self.n_train_itr:
            ...
            if self.itr % self.log_freq == 0:
                ...
                if eval_mode:
                    ...
                    log.info(
                        f"{self.itr}: step {cnt_train_step:8d} | loss {loss:8.4f} | pg loss {pg_loss:8.4f} | value loss {v_loss:8.4f}  | reward {avg_episode_reward:8.4f} | eta {eta:8.4f} | t:{time:8.4f}"
                    )
                    if self.use_wandb:
                        wandb.log(
                            {
