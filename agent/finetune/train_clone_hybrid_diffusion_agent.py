"""
Model-free online RL with DIffusion POlicy (DIPO) -- modified so actions aren't overwritten

Applies action gradient to perturb cloning actions towards maximizer of Q-function.
...
class TrainCloneHybridDiffusionAgent(TrainAgent):
    ...
    def run(self):
        ...
        # this is ground truth actions actually taken
        action_buffer = deque(maxlen=self.buffer_size)
        # new! buffer for optimized actions used for behavior cloning
        action_clone_buffer = deque(maxlen=self.buffer_size)
        ...
        for batch in dataloader_offline:
            ...
            action_clone_buffer_off = actions.cpu().numpy()
        ...
        while self.itr < self.n_train_itr:
            ...
            for step in range(self.n_steps):
                ...
                if not eval_mode:
                    for i in range(self.n_envs):
                        ...
                        action_clone_buffer.append(action_venv[i])
            ...
            if not eval_mode:
                ...
                # new! array of actions for behavior cloning
                action_clone_array = np.array(action_clone_buffer)
                ...
                for _ in range(num_batch):
                    ...
                    actions_clone_b_off = (
                        torch.from_numpy(action_buffer_clone_off[inds])
                        .float()
                        .to(self.device)
                    )
                    rewards_b_off = (
                    ...
                    actions_clone_b = (
                        torch.from_numpy(action_clone_array[inds]).float().to(self.device)
                    )
                    rewards_b = (
                    ...
                    obs_b = torch.cat([obs_b_off, obs_b], dim=0)
                    next_obs_b = torch.cat([next_obs_b_off, next_obs_b], dim=0)
                    actions_b = torch.cat([actions_b_off, actions_b], dim=0)
                    actions_clone_b = torch.cat([actions_clone_b_off, actions_clone_b], dim=0)
                    rewards_b = torch.cat([rewards_b_off, rewards_b], dim=0)
                    terminated_b = torch.cat([terminated_b_off, terminated_b], dim=0)
                    ...
                    if self.itr >= self.n_critic_warmup_itr:
                        ...
                        inds_off = np.random.choice(len(obs_buffer_off), self.batch_size)
                        
                        obs_b_off = (
                            torch.from_numpy(obs_buffer_off[inds_off]).float().to(self.device)
                        )
                        actions_clone_b_off = (
                            torch.from_numpy(action_buffer_clone_off[inds_off])
                            .float()
                            .to(self.device)
                        )

                        # get Q-perturbed actions by optimizing
                        # optimize over actions duplicated for behavior cloning instead
                        actions_flat = actions_clone_b_off.reshape(len(actions_clone_b_off), -1)
                        ...
                        # Add back to buffer
                        action_buffer_clone_off[inds_off] = guided_action_np

                        # Update policy with collected trajectories
                        loss_actor = self.model.loss(
                            guided_action.detach(), {"state": obs_b_off}
                        )
                        ...
                        # get Q-perturbed actions by optimizing
                        # optimize over actions duplicated for behavior cloning instead
                        actions_flat = actions_clone_b.reshape(len(actions_clone_b), -1)
                        actions_optim = torch.optim.Adam(
                            [actions_flat], lr=self.action_lr, eps=1e-5
                        )
                        for _ in range(self.action_gradient_steps):
                        ...
                        # Add back to buffer
                        action_clone_array[inds] = guided_action_np
                ...
                # cloned buffer only. previously, this was the actual action buffer.
                action_clone_buffer = deque(
                    [action for action in action_clone_array], maxlen=self.buffer_size
                )
